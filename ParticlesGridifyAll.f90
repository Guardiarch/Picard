subroutine ParticlesGridifyAll(F,N,C,particles, &
...
! For the implementation of non-blocking communication
  ...
  idpy = myid + (mod(myid+iprocs*jprocs+iprocs, iprocs*jprocs) - &
       mod(myid+iprocs*jprocs, iprocs*jprocs))
  idny = myid + (mod(myid+iprocs*jprocs-iprocs, iprocs*jprocs) - &
       mod(myid+iprocs*jprocs, iprocs*jprocs))
  idpz = myid + (mod(myid+iprocs*jprocs*kprocs+iprocs*jprocs, &
       iprocs*jprocs*kprocs) &
       - mod(myid+iprocs*jprocs*kprocs, iprocs*jprocs*kprocs))
  idnz = myid + (mod(myid+iprocs*jprocs*kprocs-iprocs*jprocs, &
       iprocs*jprocs*kprocs) &
       - mod(myid+iprocs*jprocs*kprocs, iprocs*jprocs*kprocs))
...
! Loop over all species and particles
  do pp = num_local, 1, -1 
     ! Find the index corresponding to the lowest x, y, z, which the
     ! particle contributes charge to.
...
! Start interpolation         
    ...
    F( particles%species(pp), 1, index_local(1)+0, index_local(2)+0, index_local(3)+0 ) = &
    F( particles%species(pp), 1, index_local(1)+0, index_local(2)+0, index_local(3)+0 ) + &
      F1*species(particles%species(pp))%n2p*particles%coordinates(4,pp)
    F( particles%species(pp), 1, index_local(1)+1, index_local(2)+0, index_local(3)+0 ) = &
    F( particles%species(pp), 1, index_local(1)+1, index_local(2)+0, index_local(3)+0 ) + &
      F2*species(particles%species(pp))%n2p*particles%coordinates(4,pp)
    F( particles%species(pp), 1, index_local(1)+0, index_local(2)+1, index_local(3)+0 ) = &
    F( particles%species(pp), 1, index_local(1)+0, index_local(2)+1, index_local(3)+0 ) + &
      F3*species(particles%species(pp))%n2p*particles%coordinates(4,pp)
    F( particles%species(pp), 1, index_local(1)+0, index_local(2)+0, index_local(3)+1 ) = &
    F( particles%species(pp), 1, index_local(1)+0, index_local(2)+0, index_local(3)+1 ) + &
      F4*species(particles%species(pp))%n2p*particles%coordinates(4,pp)
    F( particles%species(pp), 1, index_local(1)+1, index_local(2)+1, index_local(3)+0 ) = &
    F( particles%species(pp), 1, index_local(1)+1, index_local(2)+1, index_local(3)+0 ) + &
      F5*species(particles%species(pp))%n2p*particles%coordinates(4,pp)
    F( particles%species(pp), 1, index_local(1)+0, index_local(2)+1, index_local(3)+1 ) = &
    F( particles%species(pp), 1, index_local(1)+0, index_local(2)+1, index_local(3)+1 ) + &
      F6*species(particles%species(pp))%n2p*particles%coordinates(4,pp)
    F( particles%species(pp), 1, index_local(1)+1, index_local(2)+0, index_local(3)+1 ) = &
    F( particles%species(pp), 1, index_local(1)+1, index_local(2)+0, index_local(3)+1 ) + &
      F7*species(particles%species(pp))%n2p*particles%coordinates(4,pp)
    F( particles%species(pp), 1, index_local(1)+1, index_local(2)+1, index_local(3)+1 ) = &
    F( particles%species(pp), 1, index_local(1)+1, index_local(2)+1, index_local(3)+1 ) + &
      F8*species(particles%species(pp))%n2p*particles%coordinates(4,pp)

    F( particles%species(pp), 2, index_local(1)+0, index_local(2)+0, index_local(3)+0 ) = &
    F( particles%species(pp), 2, index_local(1)+0, index_local(2)+0, index_local(3)+0 ) + &
      F1*species(particles%species(pp))%n2p*particles%coordinates(5,pp)
    F( particles%species(pp), 2, index_local(1)+1, index_local(2)+0, index_local(3)+0 ) = &
    F( particles%species(pp), 2, index_local(1)+1, index_local(2)+0, index_local(3)+0 ) + &
      F2*species(particles%species(pp))%n2p*particles%coordinates(5,pp)
    F( particles%species(pp), 2, index_local(1)+0, index_local(2)+1, index_local(3)+0 ) = &
    F( particles%species(pp), 2, index_local(1)+0, index_local(2)+1, index_local(3)+0 ) + &
      F3*species(particles%species(pp))%n2p*particles%coordinates(5,pp)
    F( particles%species(pp), 2, index_local(1)+0, index_local(2)+0, index_local(3)+1 ) = &
    F( particles%species(pp), 2, index_local(1)+0, index_local(2)+0, index_local(3)+1 ) + &
      F4*species(particles%species(pp))%n2p*particles%coordinates(5,pp)
    F( particles%species(pp), 2, index_local(1)+1, index_local(2)+1, index_local(3)+0 ) = &
    F( particles%species(pp), 2, index_local(1)+1, index_local(2)+1, index_local(3)+0 ) + &
      F5*species(particles%species(pp))%n2p*particles%coordinates(5,pp)
    F( particles%species(pp), 2, index_local(1)+0, index_local(2)+1, index_local(3)+1 ) = &
    F( particles%species(pp), 2, index_local(1)+0, index_local(2)+1, index_local(3)+1 ) + &
      F6*species(particles%species(pp))%n2p*particles%coordinates(5,pp)
    F( particles%species(pp), 2, index_local(1)+1, index_local(2)+0, index_local(3)+1 ) = &
    F( particles%species(pp), 2, index_local(1)+1, index_local(2)+0, index_local(3)+1 ) + &
      F7*species(particles%species(pp))%n2p*particles%coordinates(5,pp)
    F( particles%species(pp), 2, index_local(1)+1, index_local(2)+1, index_local(3)+1 ) = &
    F( particles%species(pp), 2, index_local(1)+1, index_local(2)+1, index_local(3)+1 ) + &
      F8*species(particles%species(pp))%n2p*particles%coordinates(5,pp)

    F( particles%species(pp), 3, index_local(1)+0, index_local(2)+0, index_local(3)+0 ) = &
    F( particles%species(pp), 3, index_local(1)+0, index_local(2)+0, index_local(3)+0 ) + &
      F1*species(particles%species(pp))%n2p*particles%coordinates(6,pp)
    F( particles%species(pp), 3, index_local(1)+1, index_local(2)+0, index_local(3)+0 ) = &
    F( particles%species(pp), 3, index_local(1)+1, index_local(2)+0, index_local(3)+0 ) + &
      F2*species(particles%species(pp))%n2p*particles%coordinates(6,pp)
    F( particles%species(pp), 3, index_local(1)+0, index_local(2)+1, index_local(3)+0 ) = &
    F( particles%species(pp), 3, index_local(1)+0, index_local(2)+1, index_local(3)+0 ) + &
      F3*species(particles%species(pp))%n2p*particles%coordinates(6,pp)
    F( particles%species(pp), 3, index_local(1)+0, index_local(2)+0, index_local(3)+1 ) = &
    F( particles%species(pp), 3, index_local(1)+0, index_local(2)+0, index_local(3)+1 ) + &
      F4*species(particles%species(pp))%n2p*particles%coordinates(6,pp)
    F( particles%species(pp), 3, index_local(1)+1, index_local(2)+1, index_local(3)+0 ) = &
    F( particles%species(pp), 3, index_local(1)+1, index_local(2)+1, index_local(3)+0 ) + &
      F5*species(particles%species(pp))%n2p*particles%coordinates(6,pp)
    F( particles%species(pp), 3, index_local(1)+0, index_local(2)+1, index_local(3)+1 ) = &
    F( particles%species(pp), 3, index_local(1)+0, index_local(2)+1, index_local(3)+1 ) + &
      F6*species(particles%species(pp))%n2p*particles%coordinates(6,pp)
    F( particles%species(pp), 3, index_local(1)+1, index_local(2)+0, index_local(3)+1 ) = &
    F( particles%species(pp), 3, index_local(1)+1, index_local(2)+0, index_local(3)+1 ) + &
      F7*species(particles%species(pp))%n2p*particles%coordinates(6,pp)
    F( particles%species(pp), 3, index_local(1)+1, index_local(2)+1, index_local(3)+1 ) = &
    F( particles%species(pp), 3, index_local(1)+1, index_local(2)+1, index_local(3)+1 ) + &
      F8*species(particles%species(pp))%n2p*particles%coordinates(6,pp)
