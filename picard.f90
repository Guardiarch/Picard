...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  integer :: Nx, Ny, Nz, iteration, iter_end, iter_start, &
       ii, jj, kk, ss, num_local, num_global, &
       writeFields_iteration, writeParticles_iteration, maxit, &
       writeOutput_interval, dump_period_dump, Nretries, attempts
  real*8  :: Lx, Ly, Lz, Lx_min, Lx_max, Ly_min, Ly_max, Lz_min, Lz_max, & 
       Lx_local, Ly_local, Lz_local, xmin, xmax, ymin, ymax, &
       zmin, zmax, dxyz(3), &
       writeParticles_interval, writeFields_interval, &
...
! electrons per cell (ppc), number of particles per macroparticle (n2p)
  ...
  real*8  :: mass(8), charge(8), eta(8), kelvin(8), density(8), &
       n2p(8), tB(4,8), sB(4,8), & 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  integer, parameter :: iprocs = 6
  integer, parameter :: jprocs = 6
  integer, parameter :: kprocs = 6
...
! Physical and mathematical constants in SI units
  ...
  real*8, parameter :: mp   = 1.672621898d-27 ! proton mass [kg]
  real*8, parameter :: me   = 9.10938356d-31  ! electron mass [kg]
  ...
  real*8, parameter :: kb   = 1.38064852d-23  ! Bolzmann's constant [J/K]
...
! Number of particles for each species.
  real*8 :: N(Nspecies,Nx_local+2,Ny_local+2,Nz_local+2)

! Particle flux for each species.
  real*8 :: F(Nspecies,3,Nx_local+2,Ny_local+2,Nz_local+2)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  call GetGeneralInput(startfromdumpfile, dump_period_dump)
...
! REAL
  xmin = -720.0d0  ! physical domain
  ymin = -720.0d0  ! physical domain
  zmin = -720.0d0  ! physical domain
  xmax =  720.0d0  ! physical domain
  ymax =  720.0d0  ! physical domain
  zmax =  720.0d0  ! physical domain
  dt   =  4.0d-7   ! time step
...
! FLUID APPROXIMATION. TRUE: v_perp=FxB/(q*B^2), a_para = F dot B...
!                     FALSE: Lorentz force
  fluid(1)          = .false.     ! e-
  fluid(2)          = .false.     ! H+
  fluid(3)          = .false.     ! H2O+
...
! Micromass [kg].
  ...
  mass(2)           = mp*1.0d0                  ! H+
  mass(3)           = 18.01473d0*1.66053904d-27 ! H2O+
  mass(4)           = mp*4.0d0                  ! He++
...
! Microcharge [C]. Index 1 and 2 are electrons (-ec) and protons (+ec),
! which are calculated in Section 2.2.
  charge(3)         = ec         ! H2O+
  charge(4)         = ec         
  charge(5)         = ec         
  charge(6)         = ec
...
! UPSTREAM number density [m-3]. Index 1 are electrons and are calculated
! in Section 2.2.
  density(2)        = 7.78d5     ! 7d6 at 1 AU  ! H+
  density(3)        = 0.0d6      ! H2O+
  density(4)        = 0.0d6      
  density(5)        = 0.0d6      
  density(6)        = 0.0d6
...
! UPSTREAM number of particles per cell [#]. Index 1 are electrons and
! are calculated in Section 2.2.
  ppc(2)            = 64    ! H+
  ppc(3)            = 0     ! H2O+
  ppc(4)            = 0    
  ppc(5)            = 0    
...
! UPSTREAM temperature [K]  ( 1.1605d4 K = 1 eV )
  kelvin(1)         = 7.21d4    ! 15.0d4 at 1 AU.   ! e-
  kelvin(2)         = 3.85d4    ! 8.0d4 at 1 AU.    ! H+
  kelvin(3)         = 3.85d4    ! 8.0d4 at 1 AU.   ! H2O+
  kelvin(4)         = 0.0d4    
  kelvin(5)         = 0.0d4    
  kelvin(6)         = 0.0d4
  kelvin(7)         = 0.0d4
...
! Set magnetic field strength in x(1), y(2), z(3) in [T].
  B0(1) =  0.0d-9
  B0(2) =  400.0d0*4.9616482316929589d-9  ! 5nT is in the range observed
  B0(3) =  0.0d-9                         ! by Goetz et al. (2017).
...
! Set radial neutral velocity for the Haser model [m/s]
  vn(2) = 0.0d0     ! H+
  vn(3) = 604.5d0   ! H2O+  at 3 AU according to the Hansen et al. (2016) model
  vn(4) = 0.0d0     ! He++
  vn(5) = 0.0d0     ! CO2+
  vn(6) = 0.0d0
  vn(7) = 0.0d0
...
! Set production rate of H2O [#/s]
  ...
  ! Ion production shall be constant for r<flatradius, corresponding to b in
  ! the appendix of Nilsson et al. (2018)
  flatradius = 8.0d3/400
  
! Excess energy of end products [J]
  Ek(2) = 0.0d0  ! H+
  Ek(3) = 1.0d0*1.5d1*ec ! (round off Jesper's 15.2; 1.24d1, low activity) H2O
  Ek(4) = 0.0d0  ! He+
  Ek(5) = 0.0d0  ! CO2
  Ek(6) = 0.0d0
  Ek(7) = 0.0d0
...
! Ionization rate of species 3 [1/s]
  nu_i(2) = 0.0d0   ! H
  nu_i(3) = 9.2d-8  ! H2O (8.28d-7 at 1 AU)
  nu_i(4) = 0.0d0   ! He+
  nu_i(5) = 0.0d0   ! CO2
  nu_i(6) = 0.0d0
  nu_i(7) = 0.0d0
...
! Destruction rate of species 3 [1/s]
  nu_d(2) = 0.0d0   ! H
  nu_d(3) = 9.2d-8  ! H2O (simply the same as the ionization rate) 
  nu_d(4) = 0.0d0   ! He+
  nu_d(5) = 0.0d0   ! CO2
  nu_d(6) = 0.0d0
  nu_d(7) = 0.0d0
...
! Velocity and B-field
  ve(1) = sum( ipp(:)*(n2p(:)*mass(:)+epp(:)*n2p(1)*mass(1) + &
       hpp(:)*n2p(2)*mass(2))*v0(1,:) ) &
       / sum( ipp(:)*(n2p(:)*mass(:)+epp(:)*n2p(1)*mass(1) + &
       hpp(:)*n2p(2)*mass(2)) )
  ve(2) = sum( ipp(:)*(n2p(:)*mass(:)+epp(:)*n2p(1)*mass(1) + &
       hpp(:)*n2p(2)*mass(2))*v0(2,:) ) &
       / sum( ipp(:)*(n2p(:)*mass(:) + &
       epp(:)*n2p(1)*mass(1)+hpp(:)*n2p(2)*mass(2)) )
  ve(3) = sum( ipp(:)*(n2p(:)*mass(:)+epp(:)*n2p(1)*mass(1) + &
       hpp(:)*n2p(2)*mass(2))*v0(3,:) ) &
       / sum( ipp(:)*(n2p(:)*mass(:)+epp(:)*n2p(1)*mass(1) + &
       hpp(:)*n2p(2)*mass(2)) )
...
! Calculate plasma parameters and write warnings
  if (myid .eq. 0) then
     call WriteWarnings(mass,charge,eta,kelvin,density,v0,ve,dt,n2p, &
          dxyz,B0,Nx,Ny,Nz,xmin,xmax,ymin,ymax,zmin,zmax,&
          iprocs,jprocs,kprocs,gyration,xflow,yflow,zflow,Qn,vn, &
          Ek,nu_i,nu_d,ppc,Nspecies)
  end if
  call MPI_barrier( MPI_comm_world, ierr)
...
! Loading or initializing the particles in the domain and writing to disk
 if (startfromdumpfile) then
    ...
    call LoadDump(Nspecies, num_local, max_per_proc, &
         int_particles, real_particles, myid, iter_start, attempts, Nretries)
 else
    iter_start = 0
    call ParticlesInitialize(real_particles,int_particles, &
  ...
  if (writeParticles) then
     call MPI_barrier( MPI_comm_world, ierr)
     if (DEBUG) then
        call DumpParticles( real_particles, int_particles, num_local, &
  ...
  if (writeFields) then
     call ParticlesGridifyAll(F,N,C,real_particles,int_particles, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do iteration = iter_start+1, iter_end
    ...
    call BCinflow(real_particles,int_particles, &
         Lx_min,Ly_min,Lz_min,Lx_max,Ly_max,Lz_max,dxyz,kelvin,mass,v0,ve,&
    ...
    if (updateEfield) then
       ...
       call ParticlesGridify(N,C,real_particles,int_particles, &
            Lx_min,Lx_max,Ly_min,Ly_max,Lz_min,Lz_max,dxyz,&
            xmin,xmax,ymin,ymax,zmin,zmax,Nspecies, &
            Nx_local,Ny_local,Nz_local,max_per_proc,num_local,&
            xflow,yflow,zflow,iprocs,jprocs,kprocs,charge,n2p,myid)
...
! Here is where I shall put the call to my new probe functions. HG 2018-06-26
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (writeFields .or. writeParticles) then
       if ( ( dmod(time, writeFields_interval) .lt. dt ) .or. &
            ( dmod(time, writeParticles_interval) .lt. dt ) ) then

          call AdvanceVelocityImp(real_particles,int_particles,E,B0, &
               Lx_min,Ly_min,Lz_min,dxyz,0.5d0*dt,dt,v0,ve,eta,tB,&
               num_local,max_per_proc,Nspecies, &
               Nx_local,Ny_local,Nz_local,fluid,gyration)

          if ( (writeParticles) .and. &
               ( dmod(time, writeParticles_interval) .lt. dt ) ) then
             
             call DumpParticles( real_particles, int_particles, &
                  num_local, max_per_proc, iteration, myid )
          end if

          if ( (writeFields) .and. &
               ( dmod(time, writeFields_interval) .lt. dt ) ) then
             call ParticlesGridifyAll(F,N,C,real_particles, &
                  int_particles,Lx_min,Lx_max,Ly_min,Ly_max,Lz_min,Lz_max,dxyz,&
                  xmin,xmax,ymin,ymax,zmin,zmax,Nspecies, &
                  Nx_local,Ny_local,Nz_local,max_per_proc,num_local,&
                  xflow,yflow,zflow,iprocs,jprocs,kprocs,charge,n2p,myid)
             call DumpFields( U, N, F, E, Nx_local, Ny_local, Nz_local, &
          ...
          call AdvanceVelocity(real_particles,int_particles,E,B0, &
               Lx_min,Ly_min,Lz_min,dxyz,dt,v0,ve,eta,tB,sB,&
               num_local,max_per_proc,Nspecies, &
               Nx_local,Ny_local,Nz_local,fluid,gyration)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if ( modulo(iteration,dump_period_dump)==0 ) then
       ...
       call DumpDump(Nspecies, num_local, max_per_proc, &
            int_particles, real_particles, myid, iteration, attempts, Nretries)
       if (myid == 0) then
