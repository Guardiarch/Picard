...
!
subroutine WriteWarnings(mass,charge,eta,kelvin,density,v0,ve,dt, &
     n2p,dxyz,B0,Nx,Ny,Nz,xmin,xmax,ymin,ymax,zmin,zmax,&
     iprocs,jprocs,kprocs,gyration,xflow,yflow,zflow,Qn,vn,Ek, &
     nu_i,nu_d,ppc,Nspecies)
...
! Parameters
  ...
  integer, intent(in) :: Nx, Ny, Nz, iprocs, jprocs, kprocs, ppc(8), Nspecies
  real*8, intent(in) :: mass(8), charge(8), eta(8), density(8), &
       kelvin(8), v0(4,8), ve(4), dt, &
       n2p(8), dxyz(3), B0(4), xmin, xmax, ymin, ymax, zmin, zmax, &
       Qn(8), vn(8), Ek(8), nu_i(8), nu_d(8)
  character truth*10, lie*10

! Local variables
  real*8 :: freq, wp(3), wc(3), va, vc(3), Ld(3), rL(3), rPU(3), &
       Li(3), rM(3), CFL, sd, vth(8)
...
! fg = mv/qB, warn if fg > 1/dt
  ...
  real*8, parameter :: mp        = 1.672621898d-27
  real*8, parameter :: me        = 9.10938356d-31 
...
! Optimal dt
  ...
  dt_opt_part =dsqrt(3.0d0)**(-1)*dxyz(1)*( max(2.0d0*v0(4,1),2.0d0*v0(4,2))+ &
       dsqrt(12.0d0*tau**(-1))*max(vc(1),vc(2)) )**(-1)
...
! Begin by giving summary of parameters
  ...
  write(*,*) 'Number of species = ', Nspecies
...
! Write the parameters to an m-file
      open(unit=1,file='outp/parameters.m')

      write(1,fmt='(a,I4,a)') 'Nspecies = ', Nspecies, ';'
      ...
      if (gyration(1)) then
         write(1,*) 'gyration_1 = ', truth, ';'
      else
         write(1,*) 'gyration_1 = ', lie, ';'
      end if
      if (gyration(2)) then
         write(1,*) 'gyration_2 = ', truth, ';'
      else
         write(1,*) 'gyration_2 = ', lie, ';'
      end if
      if (gyration(3)) then
         write(1,*) 'gyration_3 = ', truth, ';'
      else
         write(1,*) 'gyration_3 = ', lie, ';'
      end if
      if (gyration(4)) then
         write(1,*) 'gyration_4 = ', truth, ';'
      else
         write(1,*) 'gyration_4 = ', lie, ';'
      end if
      if (gyration(5)) then
         write(1,*) 'gyration_5 = ', truth, ';'
      else
         write(1,*) 'gyration_5 = ', lie, ';'
      end if
      if (gyration(6)) then
         write(1,*) 'gyration_6 = ', truth, ';'
      else
         write(1,*) 'gyration_6 = ', lie, ';'
      end if
      if (gyration(7)) then
         write(1,*) 'gyration_7 = ', truth, ';'
      else
         write(1,*) 'gyration_7 = ', lie, ';'
      end if
      if (gyration(8)) then
         write(1,*) 'gyration_8 = ', truth, ';'
      else
         write(1,*) 'gyration_8 = ', lie, ';'
      end if
...
! write all warnings to a file too
     open(unit=1,file='outp/warnings.dat')
